...
class Block_dec(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.mlp = Mlp(dim=dim,hidden_dim=mlp_hidden_dim,act_layer=act_layer, drop=drop)
...
class LeFF(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # spatial restore
        x = rearrange(x, ' b (h w) (c) -> b c h w ', h = hh, w = hh2)
        # bs,hidden_dim,32x32

        x = self.dwconv(x)

        # flaten
        x = rearrange(x, ' b c h w -> b (h w) c', h = hh, w = hh2)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        # self.mlp = Mlp(dim=dim,hidden_dim=mlp_hidden_dim,act_layer=act_layer, drop=drop)
        self.LeFF = LeFF(dim=dim,hidden_dim=mlp_hidden_dim,act_layer=act_layer, drop=drop)
        self.apply(self._init_weights)
    ...
    def forward(self, x, H, W):
        ...
        x = x + self.drop_path(self.attn(self.norm1(x),H,W))
        x = x + self.drop_path(self.LeFF(self.norm2(x), H, W))
        return x
