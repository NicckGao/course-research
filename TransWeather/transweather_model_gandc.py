...
class EncoderTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.patch_embed2 = OverlapPatchEmbed(img_size=img_size // 4, patch_size=3, stride=2, in_chans=64,
                                              embed_dim=embed_dims[1])
        self.patch_embed3 = OverlapPatchEmbed(img_size=img_size // 8, patch_size=3, stride=2, in_chans=128,
                                              embed_dim=embed_dims[2])
        self.patch_embed4 = OverlapPatchEmbed(img_size=img_size // 16, patch_size=3, stride=2, in_chans=320,
        ...
        self.m1=maxpooling(img_size=img_size, patch_size=3, stride=2)
        self.m2=maxpooling(img_size=img_size, patch_size=3, stride=2)
        self.m3=maxpooling(img_size=img_size, patch_size=3, stride=2)
        # main  encoder
    ...
    def forward_features(self, x):
        ...
        y=self.m1(y)
        x1=torch.cat((x1,y.reshape(y.shape[0],x1.shape[1],1)),2)
...
class maxpooling(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4):
        ...
        self.proj = nn.MaxPool2d(kernel_size=patch_size, stride=stride,
    ...
    def forward(self, x):
        # pdb.set_trace()
        x = self.proj(x.reshape(x.shape[0],x.shape[1],x.shape[2]))
        return x
